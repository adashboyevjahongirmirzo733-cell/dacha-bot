...
async def main_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "dachas":
        ...
        if not dachas:
            await q.edit_message_text("😔 Hozircha dacha yo'q.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Orqaga", callback_data="back_main")]]))
            return MAIN
        ...
        if not bookings:
            await q.edit_message_text("📋 Sizda bron yo'q.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Orqaga", callback_data="back_main")]]))
            return MAIN
        text = "📋 *Bronlaringiz:*\n\n"
        status_map = {"pending": "⏳", "confirmed": "✅", "rejected": "❌"}
        for b in bookings:
            text += f"{status_map.get(b['status'],'❓')} *{b['dacha_name']}*\n📅 {b['start_date']} → {b['end_date']}\n💰 {b['total_price']:,} so'm\n\n"
        await q.edit_message_text(text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Orqaga", callback_data="back_main")]]))
        return MAIN
...
async def admin_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "add_dacha":
        ...
        await q.edit_message_text(f"⚠️ *{d['name']}* ni o'chirmoqchimisiz?", parse_mode="Markdown",
                                  reply_markup=InlineKeyboardMarkup([
...
async def add_price(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("4️⃣ Manzilini kiriting:\n_Misol: Toshkent viloyati, Bo'stonliq_", parse_mode="Markdown")
    return ADD_LOCATION


async def add_location(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ctx.user_data['new_location'] = update.message.text.strip()
    await update.message.reply_text("5️⃣ Rasmini yuboring:",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⏭ Rasmisiz davom etish", callback_data="skip_photo")]]))
    return ADD_PHOTO
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            ADD_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_name), CallbackQueryHandler(add_name)],
            ADD_DESC: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_desc)],
            ADD_PRICE: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_price)],
            ADD_LOCATION: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_location)],
            ADD_PHOTO: [MessageHandler(filters.PHOTO, add_photo), CallbackQueryHandler(add_photo)],
        },
